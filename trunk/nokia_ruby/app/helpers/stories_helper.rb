...
deleted = Story.find(story_id).deleted_before_type_cast
...
deleted = Story.find(story_id).deleted_before_type_cast
#There are several cases concerning the date of creation, last update and deletion of the story that has to handeled:
#1) If the story was created and deleted within the last 30 days, then I only return the flags from the table between the creation date and the last update which is the deletion:
  if deleted && creationOfStory > 30.days.ago.to_date && lastUpdated > 30.days.ago.to_date
  ...
  #2) If the story was created before the last 30 days and deleted within the last 30 days, then I only return the flags from the table between the last 30 days and the last update which is the deletion:
  elsif deleted && creationOfStory <= 30.days.ago.to_date && lastUpdated > 30.days.ago.to_date
    flags_by_day = Flag.where(:created_at => 30.days.ago..lastUpdated, :story_id => story_id).group("date(created_at)").select("created_at, count(story_id) as noOfSpamsPerDay")
    (30.days.ago.to_date..lastUpdated.to_date).map do |date|
      flag = flags_by_day.detect { |flag| flag.created_at.to_date == date }
      flag && flag.noOfFlagsPerDay.to_i || 0
    end.inspect
  #3) If the story was created and deleted before the last 30 days, then I will return an empty array:
    ...
    flags_by_day = []
  #4) If the story wasn't deleted and it was created before the last 30 days, then I return the flags from the table between the last 30 days and the current date:
  elsif creationOfStory <= 30.days.ago.to_date
    flags_by_day = Flag.where(:created_at => 30.days.ago..Time.zone.now.end_of_day, :story_id => story_id).group("date(created_at)").select("created_at, count(story_id) as noOfSpamsPerDay")
    (30.days.ago.to_date..Time.zone.now.to_date).map do |date|
      flag = flags_by_day.detect { |flag| flag.created_at.to_date == date }
      flag && flag.noOfFlagsPerDay.to_i || 0
    end.inspect
  #5) If the story wasn't deleted and it was created within the last 30 days, then I return the flags from the table between the creation date of the story and the current date:
  else
    flags_by_day = Flag.where(:created_at => creationOfStory.beginning_of_day..Time.zone.now.end_of_day, :story_id => story_id).group("date(created_at)").select("created_at, count(story_id) as noOfSpamsPerDay")
    (creationOfStory.to_date..Time.zone.now.to_date).map do |date|
      flag = flags_by_day.detect { |flag| flag.created_at.to_date == date }
...
sid = Story.where(:title => stitle).select("story_id")
...
#a method that takes list of interests and returns list of stories related to those interests
  def get_stories(interests)
#create new array to return the stories in
  @stories=Array.new
#loop for each interest and get its id then it queries the database to return all the stories which belongs to this specific interest and append them to the stories list.
  interests.each do |interest|
	 @stories += Story.find_all_by_interest_id(interest.id)
  end
