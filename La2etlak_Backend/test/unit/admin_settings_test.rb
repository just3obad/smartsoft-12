...
class Admin_SettingsTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @auto_hding = Admin_Settings.create!(:key=>"auto_hiding", :value=>0)
  end
  # author : Gasser
  test "admin should configure flags threshold" do
    Admin_Settings.configure_flags_threshold 50 , true
  ...
  test "stories should be hidden after setting the threshold" do
    stories = Story.all
    Admin_Settings.configure_flags_threshold 10 , true
  ...
  test "admin should disable auto hiding" do 
    Admin_Settings.configure_flags_threshold 10, false
    assert_equal Admin_Settings.find_by_key("auto_hiding").value, 0, "Auto Hiding was not changed"
  end

  # author : Gasser
  test "admin should not be able to configure the threshold when the auto hiding is disabled" do
    Admin_Settings.configure_flags_threshold 50, false
    assert_not_equal Admin_Settings.find_by_key("flags_threshold").value, 50, "The threshold was changed"
  end
